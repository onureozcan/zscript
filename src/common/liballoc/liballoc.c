...
#ifdef DEBUG
uint_t l_allocated = 0;		//< The real amount of memory allocated.
uint_t l_inuse = 0;			//< The amount of memory in use (malloc'ed). 
#endif


static int l_initialized = 0;            //< Flag to indicate initialization.
static int l_pageSize = 4096*100;            //< Individual page size
 ...
 */
static inline int getexp(uint_t size) {
    if (size < (1 << MINEXP)) {
#ifdef DEBUG
...
static void *liballoc_memcpy(void *s1, const void *s2, size_t n) {
    char *cdest;
    char *csrc;
    uint_t *ldest = (uint_t *) s1;
    uint_t *lsrc = (uint_t *) s2;

    while (n >= sizeof(uint_t)) {
        *ldest++ = *lsrc++;
        n -= sizeof(uint_t);
    }
...
static inline struct boundary_tag *split_tag(struct boundary_tag *tag) {
    uint_t remainder = tag->real_size - sizeof(struct boundary_tag) - tag->size;

    struct boundary_tag *new_tag =
            (struct boundary_tag *) ((uint_t) tag + sizeof(struct boundary_tag) + tag->size);
...
static struct boundary_tag *allocate_new_tag(uint_t size) {
    uint_t pages;
    uint_t usage;
...
void *lmalloc(size_t size) {
    ...
    uint_t remainder = tag->real_size - size - sizeof(struct boundary_tag) * 2; // Support a new tag + remainder
    ...
    ptr = (void *) ((uint_t) tag + sizeof(struct boundary_tag));
...
void lfree(void *ptr) {
    ...
    tag = (struct boundary_tag *) ((uint_t) ptr - sizeof(struct boundary_tag));
    ...
    if ((tag->split_left == NULL) && (tag->split_right == NULL)) {
        ...
        if (l_completePages[index] == MAXCOMPLETE) {
            ...
            uint_t pages = tag->real_size / l_pageSize;
...
void *lrealloc(void *p, size_t size) {
    ...
    tag = (struct boundary_tag *) ((uint_t) p - sizeof(struct boundary_tag));
    real_size = tag->size;
